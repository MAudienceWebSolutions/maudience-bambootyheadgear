...
class MarketPress {
	...
	function post_updated_messages( $messages ) {
		...
		if ( $post_type != 'mp_order' && $post_type != 'product' ) {
			return $messages;
		}
...
Thanks again!", 'mp' )
	...
	function store_themes_select() {
		...
		foreach ( $theme_list as $value => $name ) {
			...
			?><option value="<?php echo $value ?>"<?php selected( $this->get_setting( 'store_theme' ), $value );
			echo $disabled; ?>><?php echo $name ?></option><?php
		}
		?>
...
';
				...
				if ( isset( $meta[ "mp_price" ] ) && $meta[ "mp_price" ] ) {
					...
					foreach ( $meta[ "mp_price" ] as $key => $price ) {
						...
						<tr class="variation">
							...
							<td class="mp_custom_field_col"><input type="checkbox" class="mp_has_custom_field" name="mp_has_custom_field[<?php echo $key; ?>]" value="1" <?php checked( MP_LITE === false && isset( $meta[ 'mp_has_custom_field' ][ $key ] ) && $meta[ 'mp_has_custom_field' ][ $key ] );
				echo MP_LITE === true ? ' disabled="disabled"' : ''; ?> /></td>
	...
	//checks if a given cart has both downloadable and physical products
	function mixed_cart( $cart ) {
		$digital	 = false;
		$physical	 = false;
		foreach ( (array) $cart as $product_id => $variations ) {
			foreach ( (array) $variations as $variation => $data ) {
...
//					'base' => add_query_arg( 'apage', '%#%' ).$url2, //@todo: What is $url2???
			...
			case "presentation":
				...
				foreach ( $alignments as $value => $label ) :
					...
					?>
													<label for="<?php echo $input_id; ?>"><input value="<?php echo $value; ?>" type="radio" name="mp[image_alignment_single]" id="<?php echo $input_id; ?>" <?php checked( $this->get_setting( 'image_alignment_single' ), $value ); ?> /> <?php echo $label; ?></label><?php endforeach; ?>
											</td>
										</tr>
										<tr>
				...
				foreach ( $alignments as $value => $label ) :
					...
					?>
													<label for="<?php echo $input_id; ?>"><input value="<?php echo $value; ?>" type="radio" name="mp[image_alignment_list]" id="<?php echo $input_id; ?>" <?php checked( $this->get_setting( 'image_alignment_list' ), $value ); ?> /> <?php echo $label; ?></label><?php endforeach; ?>
											</td>
										</tr>
										</tr>
										<tr>
			...
			case "messages":
				...
				if ( $calculated_methods ) {
					...
					<?php } ?>
											...
											foreach ( (array) $mp_shipping_plugins as $code => $plugin ) {
												if ( !$plugin[ 2 ] )
													continue; //skip non calculated
												?><label><input type="checkbox" class="mp-shipping-method" name="mp[shipping][calc_methods][<?php echo $code; ?>]" value="<?php echo $code; ?>"<?php echo $this->get_setting( "shipping->calc_methods->$code" ) ? ' checked="checked"' : '';
												echo MP_LITE === true ? ' disabled="disabled"' : ''; ?> /> <?php echo esc_attr( $plugin[ 1 ] ); ?></label><br /><?php
													}
													?>
												</td>
											</tr>
