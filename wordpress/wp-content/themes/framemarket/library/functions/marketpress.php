...
function framemarket_grid_mp_list_products( $echo = true, $paginate = '', $paged = '', $per_page = '', $order_by = '', $order = '', $category = '', $tag = '', $search = '' ) {
  ...
  if ($last = count($custom_query->posts)) {
    ...
    foreach ($custom_query->posts as $post) {
      ...
      if( $mp->get_setting('show_excerpt') == 1 )
...
<hr/>
	...
	if (isset($order->mp_order_notes)) {
		echo '<div class="grid_mp_order_notes"><h3>' . apply_filters('mp_order_status_section_title_order_notes', __('Order Notes', 'mp'), $order) . '</h3>' .
			wpautop($order->mp_order_notes) . '</div>';
	}
      ?>
